...
let
  ...
  srcPatched = fetchFromGitHub {
    ...
    postFetch = ''
      sed -i '/"postinstall":/d' _build/cl-electron/package.json
      sed -i 's|(asdf:system-relative-pathname :cl-electron "source/server.js")|#P"@cl-electron-server@/source/server.js"|g' _build/cl-electron/source/core.lisp
    '';
  };
  ...
  cl-electron-server = buildNpmPackage {
    ...
    version = (builtins.fromJSON (builtins.readFile "${srcPatched}/_build/cl-electron/package.json")).version;
